...
void MenuSingleton::_bind_methods() {
	...
	OV_BIND_METHOD(
		MenuSingleton::population_menu_select_province_list_entry,
		{ "select_index", "set_scroll_index" }, DEFVAL(false)
	);
	OV_BIND_METHOD(MenuSingleton::population_menu_select_province, { "province_index" });
	OV_BIND_METHOD(
		MenuSingleton::population_menu_toggle_expanded,
		{ "toggle_index", "emit_selected_changed" }, DEFVAL(true)
	);
...
MenuSingleton::MenuSingleton()
	: population_menu { .pop_type_sort_cache { nullptr },
...
String MenuSingleton::get_province_building_identifier(int32_t building_index) const {
	...
	std::vector<BuildingType const*> const& province_building_types =
		game_singleton->get_definition_manager().get_economy_manager().get_building_type_manager().get_province_building_types(
		);
	ERR_FAIL_COND_V_MSG(
...
Dictionary MenuSingleton::get_topbar_info() const {
	...
	ret[military_rank_key] = static_cast<uint64_t>(country->get_military_rank());
	{
		...
		for (auto const& [source, power] : { std::pair { "MIL_FROM_TROOPS", country->get_military_power_from_land() },
...
Error MenuSingleton::generate_search_cache() {
	...
	for (ProvinceInstance const& province : provinces) {
		...
		search_panel.entry_cache.push_back({ &province, std::move(display_name), std::move(search_name), identifier.to_lower() }
		);
	}
	...
	for (StateSet const& state_set : state_sets) {
		for (State const& state : state_set.get_states()) {
			...
			search_panel.entry_cache.push_back({ // TODO - include state identifier? (region and/or split?)
			                                     &state,
