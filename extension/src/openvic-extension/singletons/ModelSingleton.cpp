...
Dictionary ModelSingleton::get_animation_dict(GFX::Actor::Animation const& animation) {
	...
	dict[file_key] = GameSingleton::get_singleton()->lookup_file_path(convert_to<String>(animation.get_file()));
...
Dictionary ModelSingleton::get_model_dict(GFX::Actor const& actor) {
	...
	dict[file_key] = GameSingleton::get_singleton()->lookup_file_path(convert_to<String>(actor.get_model_file()));
