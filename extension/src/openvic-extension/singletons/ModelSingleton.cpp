...
Ref<Animation> ModelSingleton::get_xsm_animation(String source_file) {
	const xsm_map_t::ConstIterator it = xsm_cache.find(source_file);
	if(it != xsm_cache.end()) {
		return it->value;
	}
	...
	xsm_cache.insert(source_file,anim);
	return anim;
}

Node3D* ModelSingleton::get_xac_model(String source_file, bool is_unit) {
	const xac_map_t::ConstIterator it = xac_cache.find(source_file);
	if(it != xac_cache.end()) {
		Node3D* unit = (Node3D*)it->value->duplicate();
	...
	xac_cache.insert(source_file,node);
...
int32_t ModelSingleton::set_unit_material_texture(int32_t type, String texture_name) {//MAP_TYPE::Values
	...
	const shader_array_index_map_t::ConstIterator it = map.find(texture_name);
	if(it != map.end()) {
		//UtilityFunctions::print(
		//	vformat("cached texture: %s type: %d, index %d", texture_name, type, it->second)
		//);
		return it->value; //return the index
	}

	int32_t index = map.size();
	map.insert(texture_name,index);
	if (map.size() >= MAX_UNIT_TEXTURES) {
		spdlog::error_s("Number of textures exceeded max supported by a shader!");
		return 0;
	}
...
int32_t ModelSingleton::set_scroll_material_texture(String texture_name) {
	const shader_array_index_map_t::ConstIterator it = scroll_index_map.find(texture_name);
	if(it != scroll_index_map.end()) {
		return it->value; //return the index
	}

	scroll_index_map.insert(texture_name,scroll_index_map.size());
